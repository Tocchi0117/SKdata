...
float ax, ay[SAMPLE_SIZE], az;


float ax_SK, ay_SK, az_SK;//合成加速度計算用
float GOUSEI_SK;          //合成加速度格納
...
//研究室
const char* ssid = "MixLab"; // ご自分のWi-FiルータのSSIDを記述します。 
const char* password = "ccino124"; // ご自分のWi-Fiルータのパスワードを記述します。
const char* saddr = "150.89.234.182"; //taeget PC address */
...
void loop() {
  ...
  if(disp_mode == MODE_A){
    ...
    M5.IMU.getAccelData(&ax,&ay[i],&az);  // IMUから加速度を取得
    ...
    M5.IMU.getAccelData(&ax_SK,&ay_SK,&az_SK);  // IMUから加速度を取得

    GOUSEI_SK=sqrt(ax_SK*ax_SK+ay_SK*ay_SK+az_SK*az_SK);  //合成加速度算出
  
    //UDP
    Udp.beginPacket(saddr, sport);
    Udp.printf("%f", GOUSEI_SK);
    Udp.endPacket();  
    delay(100);
